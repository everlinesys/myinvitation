...
import { MessageCircle } from "lucide-react"; // Optional: install lucide-react or use a plain SVG
import { FaWhatsapp } from "react-icons/fa";

export default function ConciergeService() {
  const whatsappNumber = "919400690911"; // Added country code for global compatibility
  const message = encodeURIComponent("Hello, I'm interested in a custom wedding invitation design.");
  const whatsappLink = `https://wa.me/${whatsappNumber}?text=${message}`;

  return (
    <div className="min-h-screen bg-[#FCFAF7] text-stone-900 flex items-center justify-center p-6">
      <div className="max-w-2xl w-full text-center space-y-12">
        ...
        {/* MAIN CAPTION */}
        <section className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-150">
          <h1 className="text-4xl md:text-6xl font-serif font-light leading-[1.1] tracking-tight">
            You focus on <span className="italic">the moment</span>, <br />
            we’ll handle the aesthetic.
          </h1>

          <p className="text-stone-500 text-lg md:text-xl font-light leading-relaxed max-w-lg mx-auto">
            Skip the templates. Let our designers craft a personalized digital
            experience that reflects your unique story.
          </p>
        </section>
        ...
        <div className="pt-4 animate-in fade-in slide-in-from-bottom-6 duration-1000 delay-300">
          <a
            ...
            className="inline-flex items-center gap-3 bg-stone-900 text-stone-50 px-10 py-5 rounded-full hover:bg-stone-800 transition-all duration-300 shadow-2xl hover:shadow-stone-200 group"
          >
            <span className="font-medium tracking-wide">Request Custom Design</span>
            <div className="w-px h-4 bg-stone-700 mx-1" />
            <FaWhatsapp className="w-5 h-5 text-rose-400 group-hover:scale-110 transition-transform" />
          </a>

          <p className="mt-8 text-[11px] text-stone-400 uppercase tracking-widest">
            Direct Consultation • One-on-One Service
          </p>
        </div>
